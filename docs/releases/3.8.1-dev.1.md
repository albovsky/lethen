...
The [verified baseline](https://github.com/albovsky/lethen/blob/3.8.1-dev.1/docs/validation/swift-6.4-xcode-27.md) passes 322 tests across all four targets, with no failures or compatibility skips. Clean, warm reused-index (`--skip-build`), and native fixture scans agree on 423 findings; strict clean self-scan passes. All 12 non-optional CI jobs pass on the code baseline, including macOS/Linux Swift 6.1–6.3 and Bazel 8.x/9.x. The exact versioned commit must pass its final CI gate before this draft is published.
