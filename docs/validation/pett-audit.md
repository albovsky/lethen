...
Eleven independently selected known-used declarations were asserted present and referenced: the app entry point, root composition view, UIKit notification delegate, versioned persistence schema, two nested persistent models, the user model, a repository protocol and dispatched method, and a Swift Testing suite and test method. The temporary harness scoped declarations to the app and test modules, including the nested user model rather than a same-named dependency type, and also asserted app/unit-test/UI-test module coverage. It passed and was removed from the repository.
